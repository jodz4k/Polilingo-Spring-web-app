...
public class ClanController {
	...
	public String ucenjeReciNeNaucene(Model model, HttpSession session) {
		...
		Jezik jezik = jezikService.findByNazivAndNivo(odabraniJezikNaziv, odabraniJezikNivo);
		/*
		 * Korisnik korisnik = korisnikService.findKorisnik(korisnickoIme); ne moze da
		 * se implementira jer baza nije lepo odradila entities from tables
		 */
	...
	public String cuvanjeSesijeZaPrikazClana(@RequestParam("korisnickoIme2") String korisnickoIme2, Model model,
			HttpSession session) {
		session.setAttribute("korisnickoIme2", korisnickoIme2);

		if (korisnickoIme2 != null) {
			Korisnik korisnik = korisnikService.findKorisnik(korisnickoIme2);

			model.addAttribute("korisnickoIme2", korisnik.getKorisnickoIme());
			model.addAttribute("brPrijatelja", prijateljiService.brojPrijateljaZaKorisnika(korisnik.getIdKorisnik()));
			model.addAttribute("ispisaniJeziciKojeUci", jezikService.getJezikeKorisnika(korisnik.getIdKorisnik()));
			model.addAttribute("ispunjenihCiljeva", clanImaCiljService.getBrojClanImaCilj(korisnik.getIdKorisnik()));
		}
		return "clanPrikazClana";
	}

	@GetMapping("/dodajClana")
	public String dodajClanaZaPrijatelja(Model model, HttpSession session) {
		String clan1 = (String) session.getAttribute("korisnickoIme");
		String clan2 = (String) session.getAttribute("korisnickoIme2");
		
		System.out.println("prvi " + clan1 + " " + clan2);
		Clan clanPrvi = clanService.findByKorisnickoIme(clan1);
		Clan clanDrugi = clanService.findByKorisnickoIme(clan2);
		
		prijateljiService.dodajPrijatelja(clanPrvi.getIdClan(), clanDrugi.getIdClan());		
			
		return "clan";
	}
	
	@GetMapping("/ispisiCiljeve")
	public String ispisiCiljeve(Model model, HttpSession session) {
		model.addAttribute("nazivi", ciljService.getCiljevi());
		return "clanCiljevi";
	}
